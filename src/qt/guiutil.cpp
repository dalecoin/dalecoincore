...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("dalecoin"))
        return false;
...
{
    // Convert dalecoin:// to dalecoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("dalecoin://"))
    {
        uri.replace(0, 12, "dalecoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "DaleCoin.lnk";
}
...
{
    return GetAutostartDir() / "dalecoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("DaleCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  dalecoin-qt [" + tr("command-line options") + "]                     " + "\n";
...
void SetDaleCoinThemeQSS(QApplication& app)
{
...
void SetDaleCoinFonts(QWidget * w) {
#ifdef Q_OS_MAC
    QFont font("Roboto", 15);
#else
    QFont font("Roboto", 11);
